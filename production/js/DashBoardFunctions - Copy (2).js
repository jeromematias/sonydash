var WeeksPeriod = [], MonthsPeriod = [], QuartersPeriod = [], YearsPeriod = [], magicType = 'bar';
...
function getPeriodDate(p, y, periodtype) {
    ...
    if($('#periodtype').val() == 1){        
        ...
        if (dow <= 4){
            ...
            ISOweekStart.setDate(periodStart.getDate() + 8 - periodStart.getDay());
        }
        ...
        if (dow <= 4){
            ...
            ISOweekEND.setDate(periodEndDate.getDate() + 8 - periodEndDate.getDay());
        }

        var temp = $('#area').val();
        for( var i in GetLastDates ){
            for(var d = 0; d < GetLastDates.length; d++){
                if(temp == GetLastDates[d].AreaID){
                    lastDate = lastDate = new Date(GetYearNum(GetLastDates[d].MaxPeriod), GetWeekNum(GetLastDates[d].MaxPeriod)-1, GetYearDay(GetLastDates[d].MaxPeriod)-1);
                }                
            }
        }
        if( lastDate.getTime() < ISOweekEND.getTime() && lastDate.getTime() > ISOweekStart.getTime()){
            var ddd = lastDate.getDate();
            var mmm = lastDate.getMonth()+1;
            var yyy = lastDate.getFullYear();
            var timeDiff = Math.abs(lastDate.getTime() - ISOweekStart.getTime());
            var diffDays = Math.ceil(timeDiff / (1000 * 3600 * 24)); 
            $('#PeriodInfo').text((diffDays+ 1) + " Days,"+dd + "-" + mm + "-" + yy + " - " +ddd + "-" + mmm + "-" + yyy);
        }else if(lastDate.getTime() < ISOweekEND.getTime() && lastDate.getTime() < ISOweekStart.getTime()){
            $('#PeriodInfo').text("0 Days,"+dd + "-" + mm + "-" + yy + " - " + "No Data");
        }else{
        ...
        if( lastDate.getTime() < periodEndDate.getTime() && lastDate.getTime() > periodDate.getTime() ){
            var ddd = lastDate.getDate();
            var mmm = lastDate.getMonth()+1;
            var yyy = lastDate.getFullYear();
            var timeDiff = Math.abs(lastDate.getTime() - periodDate.getTime());
            var diffDays = Math.ceil(timeDiff / (1000 * 3600 * 24)); 
            $('#PeriodInfo').text((diffDays+ 1) + " Days,"+dd + "-" + mm + "-" + yy + " - " +ddd + "-" + mmm + "-" + yyy);
        }else if(lastDate.getTime() < periodEndDate.getTime() && lastDate.getTime() < periodDate.getTime()){
            $('#PeriodInfo').text("0 Days, " + dd + "-" + mm + "-" + yy + " - No Data");
        }else{
        ...
        for( var i in GetLastDates ){
            for(var d = 0; d < GetLastDates.length; d++){
                if(temp == GetLastDates[d].AreaID){
                    lastDate = lastDate = new Date(GetYearNum(GetLastDates[d].MaxPeriod), GetWeekNum(GetLastDates[d].MaxPeriod)-1, GetYearDay(GetLastDates[d].MaxPeriod));
                }                
            }
        }
        if( lastDate.getTime() < periodEndDate.getTime() && lastDate.getTime() > periodDate.getTime() ){
...
function ratings(targetgroupname, channelbrandid, unitid, periodtypeid, periodnumber, refperiods, isPrimeTime,RegionID) {            
        $.ajax({
            url: "RatingPerformance.php?targetgroupname=" + targetgroupname + "&channelbrandid=" + channelbrandid + "&unitid=" + unitid + "&periodtypeid=" + periodtypeid + "&periodnumber=" + periodnumber + "&refperiods=" + refperiods + "&isPrimeTime=" + isPrimeTime+ "&RegionID=" + RegionID,
            type: "GET"
        })
        .done(function(data) {            
            var expotRows = [];
